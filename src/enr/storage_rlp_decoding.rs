...
impl Storage {
    pub(crate) fn from_rlp<S: Scheme>(bytes: &[u8]) -> Result<Storage, RlpDecodingError> {
        let mut list_decoder =
            ListDecoder::from_rlp_data(bytes).map_err(RlpDecodingError::DecodingRLPError)?;
        let mut builder = Builder::new();

        // signature
        match list_decoder.next() {
            None => {
                return Err(RlpDecodingError::InvalidFormat);
            }
            Some((item_type, payload)) => {
                let signature = rlp::decode_payload(item_type, payload)
                    .map_err(RlpDecodingError::DecodingRLPError)?;
                builder.with_signature_value(signature);
            }
        }
        ...
        match list_decoder.next() {
            ...
            Some((item_type, payload)) => {
                let seq = rlp::decode_payload(item_type, payload)
                    .map_err(RlpDecodingError::DecodingRLPError)?;
        ...
        // pairs
        while let Some((key_item_type, key_payload)) = list_decoder.next() {
            let key: Vec<u8> = rlp::decode_payload(key_item_type, key_payload)
                .map_err(RlpDecodingError::DecodingRLPError)?;
            if key <= previous_key {
            ...
            let (value_item_type, value_payload) =
                list_decoder.next().ok_or(RlpDecodingError::InvalidPair)?;

            match key.as_ref() {
                _ if key == S::public_key_key() => {
                    let public_key_value = rlp::decode_payload(value_item_type, value_payload)
                        .map_err(RlpDecodingError::DecodingRLPError)?;
                    builder.with_public_key_value(public_key_value);
                }
                ID_KEY => {
                    let id: Vec<u8> = rlp::decode_payload(value_item_type, value_payload)
                        .map_err(RlpDecodingError::DecodingRLPError)?;
                    if id != S::id() {
                ...
                IP4_KEY => {
                    let ip_octets: [u8; 4] = rlp::decode_payload(value_item_type, value_payload)
                        .map_err(|_| RlpDecodingError::InvalidIp4Octets)?;
                    builder.with_ip4(Ipv4Addr::from(ip_octets));
                }
                TCP4_KEY => {
                    let tcp = rlp::decode_payload(value_item_type, value_payload)
                        .map_err(RlpDecodingError::DecodingRLPError)?;
                    builder.with_tcp4(tcp);
                }
                UDP4_KEY => {
                    let udp = rlp::decode_payload(value_item_type, value_payload)
                        .map_err(RlpDecodingError::DecodingRLPError)?;
                    builder.with_udp4(udp);
                }
                IP6_KEY => {
                    let ip6_octets: [u8; 16] = rlp::decode_payload(value_item_type, value_payload)
                        .map_err(|_| RlpDecodingError::InvalidIp6Octets)?;
                    builder.with_ip6(Ipv6Addr::from(ip6_octets));
                }
                TCP6_KEY => {
                    let tcp6 = rlp::decode_payload(value_item_type, value_payload)
                        .map_err(RlpDecodingError::DecodingRLPError)?;
                    builder.with_tcp6(tcp6);
                }
                UDP6_KEY => {
                    let udp6 = rlp::decode_payload(value_item_type, value_payload)
                        .map_err(RlpDecodingError::DecodingRLPError)?;
                    builder.with_udp6(udp6);
                }
                _ => {
                    // unknown pair
                    if key_item_type == RlpItemType::List {
...
pub enum RlpDecodingError {
    ...
    DecodingRLPError(#[source] crate::rlp::DecodingError),
...
mod tests {
    ...
    fn test_decoding_with_spec_sample_node() {
        ...
        let storage = Storage::from_rlp::<Schemev4>(&bytes).unwrap();
    ...
    fn test_decoding_double_conversion(
        ...
        let storage = Storage::from_rlp::<Schemev4>(&rlp_data[0..size]).unwrap();
    ...
    fn test_decoding_errors() {
        let data = [
            ("01", RlpDecodingError::DecodingRLPError(rlp::DecodingError::InvalidFormat)),
            ("c3", RlpDecodingError::DecodingRLPError(rlp::DecodingError::InvalidFormat)),
            ("d291636f6e74656e745f7369676e6174757265", RlpDecodingError::MissingSeq),
            ("dc91636f6e74656e745f7369676e617475726589020000000000000000", RlpDecodingError::DecodingRLPError(rlp::DecodingError::InvalidFormat)), // invalid seq
            ...
            ("db91636f6e74656e745f7369676e6174757265018374637083100000", RlpDecodingError::DecodingRLPError(rlp::DecodingError::InvalidFormat)), // invalid tcp
            ("db91636f6e74656e745f7369676e6174757265018375647083100000", RlpDecodingError::DecodingRLPError(rlp::DecodingError::InvalidFormat)), // invalid udp
            ("e991636f6e74656e745f7369676e61747572650183697036911234567812345678123456781234567800", RlpDecodingError::InvalidIp6Octets),
            ("dc91636f6e74656e745f7369676e617475726501847463703683100000", RlpDecodingError::DecodingRLPError(rlp::DecodingError::InvalidFormat)), // invalid tcp6
            ("dc91636f6e74656e745f7369676e617475726501847564703683100000", RlpDecodingError::DecodingRLPError(rlp::DecodingError::InvalidFormat)), // invalid udp6
            ...
            ("eb91636f6e74656e745f7369676e61747572650182696482763489736563703235366b3183010203837a6970", RlpDecodingError::InvalidPair),
        ];
        for (rlp_data_hex, err) in data {
            let rlp_data = hex::decode(rlp_data_hex).unwrap();
            let result = Storage::from_rlp::<Schemev4>(&rlp_data);
    ...
    fn test_decoding_with_unknown_pair() {
        let bytes =
            hex!("")
                .to_vec();
        assert!(Storage::from_rlp::<Schemev4>(&bytes).is_ok())
    }

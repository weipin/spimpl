...
#[inline]
pub fn decode_data(data: &[u8]) -> Result<(RlpItemType, &[u8]), DecodingError> {
...
#[inline]
pub fn decode_list_payload(
    list_payload: &[u8],
) -> Result<Vec<(RlpItemType, &[u8])>, DecodingError> {
    ...
    loop {
        ...
        if remaining_list_payload.len() < header_byte_length as usize + payload_byte_length as usize
        {
            return Err(DecodingError::InvalidFormat);
        }
        let (data1, data2) = remaining_list_payload
            .split_at(header_byte_length as usize + payload_byte_length as usize);
        items.push((item_type, &data1[header_byte_length as usize..]));
...
#[inline]
pub(crate) fn decode_data_header(
    data: &[u8],
) -> Result<
    (
        RlpItemType,
        ByteLengthOfPayloadByteLength,
        PayloadByteLength,
    ),
    DecodingError,
> {
    ...
    match first {
        ...
        0x80..=0xb7 => Ok((
            ...
            1,
            (first - 0x80) as PayloadByteLength,
        )),
        ...
        0xc0..=0xf7 => Ok((RlpItemType::List, 1, (first - 0xc0) as PayloadByteLength)),

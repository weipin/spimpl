from discv5_packet import _pack_handshake_message_with_record, \
    _build_handshake_message_header_with_record, _pack_header, _pack_message
...
def invalid_masking_iv():
    packet = _pack_handshake_message_with_record(NONCE, SRC_NODE_ID, DEST_NODE_ID,
                                                 MASKING_IV, INITIATOR_KEY, MESSAGE_TYPE,
                                                 MESSAGE_RLP_ENCODED,
                                                 ID_SIGNATURE, EPH_PUBKEY, RECORD_RLP_ENCODED)
...
def invalid_flag_unexpected_value():
    flag = bytes([77])
    header = _build_handshake_message_header_with_record(NONCE, SRC_NODE_ID, ID_SIGNATURE,
                                                         EPH_PUBKEY, RECORD_RLP_ENCODED, flag=flag)
    packed_header = _pack_header(MASKING_IV, DEST_NODE_ID, header)
    packed_message = _pack_message(INITIATOR_KEY, NONCE, MASKING_IV, header, MESSAGE_TYPE,
                                   MESSAGE_RLP_ENCODED)
...
# len(RECORD_RLP_ENCODED) = 127
# 131 + 127 = 258 = bytes([1, 2])
def invalid_authdata_size_incorrect_value_a():
    authdata_size = bytes([1, 1])
    header = _build_handshake_message_header_with_record(NONCE, SRC_NODE_ID, ID_SIGNATURE,
                                                         EPH_PUBKEY, RECORD_RLP_ENCODED,
